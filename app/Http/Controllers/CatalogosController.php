...
class CatalogosController extends Controller
{
    ...
    public function clientesActualizarGet($id_clientes): View
    {
        $cliente = Clientes::findOrFail($id_clientes);

        return view('catalogos.clientesActualizar', [
            'cliente' => $cliente,
            'breadcrumbs' => [
                'Inicio' => url('/'),
                'Clientes' => route('clientes.get'),
                'Actualizar Cliente' => url("/catalogos/clientes/actualizar/{$id_clientes}")
            ]
        ]);
    }

    public function clientesActualizarPost(Request $request, $id_clientes): RedirectResponse
    {
        $validated = $request->validate([
            'nombre' => 'required|string|max:255',
            'telefono' => 'required|string|max:15',
    ...
    public function serviciosAgregarPost(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            return redirect()->route('servicios.get')->with('success', 'Servicio agregado correctamente.');
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Error al agregar el servicio: ' . $e->getMessage())->withInput();
        }
    }
    ...
    public function serviciosActualizarGet($id_servicios)
    {
        $servicio = Servicios::findOrFail($id_servicios);

        return view('catalogos.serviciosActualizarGet', [
            'servicio' => $servicio,
            'breadcrumbs' => [
                'Inicio' => url('/'),
                'Servicios' => route('servicios.get'),
                'Actualizar Servicio' => url("/catalogos/servicios/actualizar/{$id_servicios}")
            ]
        ]);
    }

    public function serviciosActualizarPost(Request $request, $id_servicios)
    {
        $validated = $request->validate([
            'nombre' => 'required|string|max:255',
            'descripcion' => 'required|string|max:500',
